...
def augment_data_at_rate_and_length_uniform(X_val_train, y_label_train, n_clones, fp_rate_max, fn_rate_max, hard_fn_flag = False, noise_std=0.3):
    ...
    for _ in range(n_clones):
        ...
        fp_rate_sampled =  np.random.uniform(0, fp_rate_max)
        fn_rate_sampled =  np.random.uniform(0, fn_rate_max)
...
def train_one_split(split_id, fp_rate_mean, fn_rate_mean, x_noisy_samples):
    ...
    X_augmented, y_augmented = augment_data_at_rate_and_length_uniform(
        ...
        hard_fn_flag=hard_fn_flag, noise_std=noise_std
    )
